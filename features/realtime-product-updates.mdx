...
Instead of polling the product catalog for changes, you can use the `product.update.v2` webhook to get real-time update pushed to your system when a product is added, updated or removed from your catalog.

<Warning>
  The original `product.update` event is deprecated. This guide covers `product.update.v2`.
</Warning>

## How to use the `product.update.v2` webhook
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    href="/reference/2024-02-05/webhook/product.update.v2"
    horizontal
    arrow
  >
    The specific details of the `product.update.v2` webhook event
  </Card>
</CardGroup>
...
The price you pay is the face value multiplied by the `price_multiplier`, before any fees. The multiplier is equal to 1 minus your discount, so a 10% discount gives `"0.9"` and a \$50 gift card costs you \$45.

In the example below the discount deepens from 5% to 10%: the multiplier moves from `"0.95"` to `"0.90"`.
