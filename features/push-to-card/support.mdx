...
3. Recipient bank has rejected payment
   ...
   - The funds are not returned to your account balance. Instead they are returned to the payout link, which remains active so the recipient can attempt redemption again with a different card.

<Note>
  Payments usually reach the recipient's bank account shortly after redemption.
  If there is a problem the payment may be delayed by up to a day before it
