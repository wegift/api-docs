...
- Ensure you have funded your account with the required balance to cover the cards you intend to issue. You can use the [balances endpoint](/features/balances) to check your account balance.
...
- For **Reload Cards only**, you can use the `RECIPIENT` distribution method. Specify a unique `recipient_reference` to identify the recipient. For subsequent orders, use the same `recipient_reference` and we will automatically reload the existing card.
