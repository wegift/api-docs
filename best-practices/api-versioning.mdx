...
When we need to introduce a backwards-incompatible change, we will create a new dated version which clients can migrate to when ready. The [API changelog](/changelog) details both major dated version releases and improvements made within each version.
...
1. **Refer to [the changelog](/changelog)** to see what contract changes the new version introduces. You can also view the full documentation for a chosen version using the dropdown in the header of this site.
