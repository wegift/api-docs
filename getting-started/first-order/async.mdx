...
You can use our [Order Completion webhook](https://developer.runa.io/docs/order-completion) to be notified when an order has completed processing.

Alternatively, you can poll the [Get Order](https://developer.runa.io/reference/getsingleorder) endpoint in order to verify the successful order processing. You will need the order ID obtained from the response received after calling the [Create Order](https://developer.runa.io/v2023-07-27/reference/createorder) endpoint. In the example response provided above, the order ID is `O-ABC123`. Simply include this order ID as a path parameter in the following URL format: `https://api.runa.io/v2/order/{id}`. Replace `{id}` with your specific order ID.

To try out this endpoint, you can visit the [Get Order](https://developer.runa.io/reference/getsingleorder) page and insert your order ID into the id path parameter.

![](https://files.readme.io/7c68558-image.png)
